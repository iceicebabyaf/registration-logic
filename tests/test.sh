...
UPDATE_BALANCE=$(curl -X POST "http://127.0.0.1:8000/update_balance?email=test@example.com&amount=100.0" \
  -H "accept: application/json" \
  -w "%{http_code}\n" -o /dev/null)
...
GET_DATA=$(curl -X GET "http://127.0.0.1:8000/get_users_db" \
  -H "accept: application/json" \
  -w "%{http_code}\n" -o /dev/null)
...
# Тест выхода
LOGOUT_STATUS=$(curl -X POST "http://127.0.0.1:8000/user_logout?email=test@example.com" \
  -H "accept: application/json" \
  -w "%{http_code}\n" -o /dev/null)
