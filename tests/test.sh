...
# GET_DATA=$(curl curl "http://127.0.0.1:8000/get_users_db" \
#   -H "accept: application/json" \
#   -w "%{http_code}\n" -o /dev/null)
...
# Тест регистрации
REG_STATUS=$(curl -X POST "http://fastapi_app:8000/user_registration?email=test@example.com&password=testpass" \
  -H "accept: application/json" \
  -w "%{http_code}\n" -o /dev/null)
if [ "$REG_STATUS" != "200" ]; then
  echo "Registration failed with status $REG_STATUS"
...
# Тест входа
LOGIN_STATUS=$(curl -X POST "http://fastapi_app:8000/user_login?email=test@example.com&password=testpass" \
  -H "accept: application/json" \
  -w "%{http_code}\n" -o /dev/null)
if [ "$LOGIN_STATUS" != "200" ]; then
  echo "Login failed with status $LOGIN_STATUS"
...
UPDATE_BALANCE=$(curl -X POST "http://fastapi_app:8000/update_balance?email=test@example.com&amount=100.0" \
  -H "accept: application/json" \
  -w "%{http_code}\n" -o /dev/null)
if [ "$UPDATE_BALANCE" != "200" ]; then
  echo "Update balance failed with status $UPDATE_BALANCE"
...
GET_DATA=$(curl -X GET "http://fastapi_app:8000/get_users_db" \
  -H "accept: application/json" \
  -w "%{http_code}\n" -o /dev/null)
if [ "$GET_DATA" != "200" ]; then
...
# Тест выхода
LOGOUT_STATUS=$(curl -X POST "http://fastapi_app:8000/user_logout?email=test@example.com" \
  -H "accept: application/json" \
  -w "%{http_code}\n" -o /dev/null)
